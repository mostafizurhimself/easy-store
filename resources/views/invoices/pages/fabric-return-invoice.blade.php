...
<main class="flex-grow-1">
    <div class="row contacts">
        ...
        <div class="col text-right">
            ...
            @endif
            <div class="email"><a href="mailto:{{$invoice->supplier->email}}">{{$invoice->supplier->email}}</a></div>
            <div class="email"><a href="tel:{{$invoice->supplier->mobile}}">{{$invoice->supplier->mobile}}</a></div>
        </div>
    </div>
    <div class="row pt-3">
        ...
        <tbody>
            @foreach ($invoice->returnItems as $item)
                <tr>
                    ...
                    <td class="text-left">
                        <h3>{{$item->fabric->name}}({{$item->fabric->code}})</h3>
                        <span>{!! $item->note !!}</span>
                    </td>
                    <td class="unit">{{Helper::currencyShort($item->rate)}}</td>
                    <td class="tax">{{$item->quantity}} {{$item->unit}}</td>

    ...
    @foreach($navigation as $group => $resources)
        ...
        <ul class="list-reset mb-8">
            @foreach($resources as $resource)
                <li class="flex items-center font-normal text-white mb-4 text-base no-underline dim">
                    <router-link :to="{
                        name: 'index',
                        params: {
                        ...
                        @if(method_exists($resource, 'icon'))
                            <i class="{{$resource::icon()}}" style="min-width:1.85rem"></i>
                        @else
                        ...
                        {{-- Menu Text --}}
                        @if(method_exists($resource, 'navigationLabel'))
